...
voteContainer.removeEventListener('click', checkImage);
	...
	headers=new Array(Selection.allUsers.length)
	votes=new Array(Selection.allUsers.length)
	
	
    for(var t=0;t<Selection.allUsers.length;t++)
	{
     //     op+="<li>" + Selection.allUsers[t].clicks + " Votes For " +  Selection.allUsers[t].name + "</li>";
		  headers[t]=Selection.allUsers[t].name;
		  votes[t]=Selection.allUsers[t].clicks;
 	} 
...
var chart = new Chart(ctx, {
    ...
    data: {
        ...
        datasets: [{
            ...
            data: votes,
            backgroundColor: ['#000010', '#444414', '#888888', '#f3f3f2']
        }]
    },
    options: {}
});
